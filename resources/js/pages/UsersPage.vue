...
const form = reactive({ id: null, name: '', email: '', role: 'user', password: '', password_confirmation: '' });
const errors = ref({});
...
function reset() { Object.assign(form, { id: null, name: '', email: '', role: 'user', password: '', password_confirmation: '' }); errors.value = {}; }
function openCreate() { editing.value = null; reset(); showForm.value = true; }
function openEdit(u) {
  editing.value = u.id; errors.value = {};
  Object.assign(form, { id: u.id, name: u.name, email: u.email, role: u.role, password: '', password_confirmation: '' });
  showForm.value = true;
}
async function save() {
  errors.value = {};
...
<template>
  ...
  <section class="surface overflow-hidden">
    <table class="data-table">
      ...
      <tbody>
        <tr v-if="!list.length"><td colspan="6" class="text-center py-8 text-ink-500">{{ loading ? 'Memuat…' : 'Belum ada pengguna.' }}</td></tr>
